...
export default function TierListPage() {
  ...
  console.log('TierListId:', tierListId);

  const [isLoading, setIsLoading] = useState(true);
  const [isMounted, setIsMounted] = useState(false);
  const [rows, setRows] = useState<TierRow[]>([]);
  const [franchiseRows, setFranchiseRows] = useState<FranchiseTierRow[]>([]);
  ...
  const loadTierList = useCallback(async () => {
    console.log('loadTierList called with tierListId:', tierListId);
    if (!tierListId) {
      console.log('No tierListId available');
      return;
    }

    setIsLoading(true);
    try {
      // Charger les informations de base de la tier list
      console.log('Fetching tier list data...');
      const tierListRes = await fetch(`/api/tierlists/${tierListId}`);
      if (!tierListRes.ok) throw new Error('Erreur lors du chargement de la tier list');
      const tierList = await tierListRes.json();
      console.log('Tier list data:', tierList);

      if (!tierList || !tierList.id) {
        throw new Error('Données de tier list invalides');
      }

      setTierListName(tierList.name);
      setIsPublic(tierList.is_public);
      setTierListType(tierList.type);
      console.log('tierListType:', tierList.type);

      // Charger les colonnes
      console.log('Fetching columns...');
      const columnsRes = await fetch(`/api/tierlists/${tierListId}/columns`);
      if (!columnsRes.ok) throw new Error('Erreur lors du chargement des colonnes');
      const columns: TierListColumn[] = await columnsRes.json();
      console.log('Columns data:', columns);

      if (!columns || !Array.isArray(columns)) {
        throw new Error('Données de colonnes invalides');
      }

      // Charger les items
      console.log('Fetching items...');
      const itemsRes = await fetch(`/api/tierlists/${tierListId}/items`);
      if (!itemsRes.ok) throw new Error('Erreur lors du chargement des items');
      const items: TierListItem[] = await itemsRes.json();
      console.log('Items data:', items);

      if (!items || !Array.isArray(items)) {
        throw new Error('Données d\'items invalides');
      }
      ...
      toast({
        title: 'Erreur',
        description: 'Impossible de charger la tier list',
        variant: 'destructive'
      });
    } finally {
      setIsLoading(false);
  ...
  useEffect(() => {
    console.log('Data loading effect triggered, isMounted:', isMounted);
    if (!isMounted) {
      console.log('Component not mounted yet, skipping data load');
      return;
    }

    loadTierList();
  }, [isMounted, loadTierList]);
            ...
            if (franchise) {
              setFranchiseRows(rows => rows.map((row, i) => {
                if (i === fromRowIdx) {
                  ...
                  return { ...row, games: row.games.some(g => g.id === franchise.id) ? row.games : [...row.games, franchise] };
                }
                return row;
              }));
            }
  ...
  return null; // Fallback si le type n'est pas encore chargé
}