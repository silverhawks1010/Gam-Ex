...
-- Recréation des tables sans contraintes de clé étrangère
...
-- Ajout des contraintes de clé étrangère dans un ordre spécifique
ALTER TABLE game_list_items
    ADD CONSTRAINT fk_list
    FOREIGN KEY (list_id)
    REFERENCES game_lists(id)
    ON DELETE CASCADE;

ALTER TABLE game_list_shares
    ADD CONSTRAINT fk_list
    FOREIGN KEY (list_id)
    REFERENCES game_lists(id)
    ON DELETE CASCADE;

-- Ajout des contraintes vers auth.users en dernier
